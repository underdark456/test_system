...
from src.enum_types_constants import StationModes, RouteTypes, CheckboxStr, RouteIds, FaultCodes, Checkbox, \
    LanCheckModes
...
class SrMaxTxFaultCase(CustomTestCase):
    """Check teleport switching due to max tx fault controllers"""

    __author__ = 'vpetuhova'
    __version__ = '4.0.0.26'
    __review__ = 'dkudryashov'
    __execution_time__ = 580  # approximate test case execution time in seconds
    ctrl_telnet = None

    @classmethod
    def set_up_class(cls):
        nms_options = test_api.get_nms()
        controllers, stations = test_api.get_uhp_controllers_stations(2, ['UHP200', ], 1, ['UHP100', ])
        ...
        nms_api.update(cls.sr_controller, {'enable': Checkbox.ON})
    ...
    def test_max_tx_fault(self):
        # Allow one TX controller in fault state (no teleport reconfiguring)
        ...
        # Check state: ctrl in fault state (Qos), Device states are Up and Redundant
        ...
        if up_dev_st != Device.FAULT or red_dev_st != Device.REDUNDANT or faults != FaultCodes.QOS:
            test_api.fail(self, f'Max_tx_fault=1: {state_up_device} state {up_dev_st}, '
                                f'{state_red_device} state {red_dev_st}, MF hub faults {faults}')

        nms_api.update(self.mf_hub, {
            'sm_enable': Checkbox.OFF
        })
        time.sleep(60)
        ...
        if up_dev_st != Device.UP or red_dev_st != Device.REDUNDANT:
            test_api.fail(self, f'Max_tx_fault=1: devices states have been changed - {state_up_device} is {up_dev_st}'
        ...
        if up_dev_st != Device.REDUNDANT or red_dev_st != Device.UP or red_dev_ctr != 'controller:0 test_ctrl':
            test_api.fail(self, f'Max_tx_fault=0: devices have not been switched - {state_up_device} is {up_dev_st},'
                                f'{state_red_device} is {red_dev_st}, {state_red_device} controller is {red_dev_ctr}')
