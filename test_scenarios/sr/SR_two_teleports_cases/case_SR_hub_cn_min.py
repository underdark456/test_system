...
class SrHubCnMinCase(CustomTestCase):
    """Check teleport switching due to Low_hub_CN fault"""

    __author__ = 'vpetuhova'
    __version__ = '0.1'
    __review__ = 'dkudryashov'
    __execution_time__ = 405  # approximate test case execution time in seconds
    ctrl_telnet = None
    ...
    def set_up_class(cls):
        ...
        # if not (state1 == 'Up' and state2 == 'Redundant') and not \
        #         (state1 == 'Redundant' and state2 == 'Up'):
        #     test_api.error(f'Devices\' states should be Up and Redundant, current states {state1} and {state2}')
    ...
    def test_sr_hub_cn_min(self):
        state1 = nms_api.get_param(self.device1, 'state')
        state2 = nms_api.get_param(self.device2, 'state')
        nms_api.update(self.sr_controller, {'check_cn': CheckboxStr.ON, 'hub_cn_min': '30'})

        time.sleep(80)
        if not nms_api.get_param(self.sr_controller, 'tp_fail_code') == 'Low_hub_CN':
        ...
        nms_api.update(self.sr_controller, {'check_cn': CheckboxStr.OFF})
