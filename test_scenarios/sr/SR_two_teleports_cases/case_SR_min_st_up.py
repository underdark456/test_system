...
class SrMinStUpCase(CustomTestCase):
    """Check teleport switching due to min st up fault controllers"""
    ...
    def test_sr_min_st_up(self):
        state1 = nms_api.get_param(self.device1, 'state')
        state2 = nms_api.get_param(self.device2, 'state')
        nms_api.update(self.sr_controller, {'check_stn': Checkbox.ON, 'min_stn_up': '70'})

        time.sleep(80)
        if not nms_api.get_param(self.sr_controller, 'tp_fail_code') == 'Down_stations':
            test_api.fail(self, f'There is no fail code `Down_stations` in SR controller')
        time.sleep(20)
