...
class SrStCnMinCase(CustomTestCase):
    """Check teleport switching due to Low_stations_CN fault"""

    __author__ = 'vpetuhova'
    __version__ = '4.0.0.26'
    __review__ = 'dkudryashov'
    __execution_time__ = 410  # approximate test case execution time in seconds
    ctrl_telnet = None
    ...
    def test_sr_st_cn_min(self):
        state1 = nms_api.get_param(self.device1, 'state')
        state2 = nms_api.get_param(self.device2, 'state')
        nms_api.update(self.sr_controller, {'check_cn': Checkbox.ON, 'stn_cn_min': '30'})

        time.sleep(80)
        if not nms_api.get_param(self.sr_controller, 'tp_fail_code') == 'Low_stations_CN':
            test_api.fail(self, 'There is no fail code `Low_stations_CN` in SR controller')
        time.sleep(20)
