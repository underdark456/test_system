...
class SrMinCtrUpCase(CustomTestCase):
    """Check teleport switching due to min ctr up fault"""
    ...
    def test_sr_min_ctr_up(self):
        state1 = nms_api.get_param(self.device1, 'state')
        state2 = nms_api.get_param(self.device2, 'state')
        nms_api.update(self.sr_controller, {'check_stn': Checkbox.ON, 'min_ctr_up': '70'})

        time.sleep(80)
        if not nms_api.get_param(self.sr_controller, 'tp_fail_code') == 'Ctr_down_stations':
            test_api.fail(self, 'There is no fail code `Ctr_down_stations` on teleport')
        time.sleep(20)
        ...
        nms_api.update(self.sr_controller, {'check_stn': Checkbox.OFF})
