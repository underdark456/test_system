...
class SrDynLicGroupsCase(CustomTestCase):
    """Check that one dynamic licenses isn't attached to two UHPs"""

    __author__ = 'vpetuhova'
    __version__ = '4.0.0.21'
    __review__ = 'dkudryashov'
    __execution_time__ = 215  # approximate test case execution time in seconds
    __express__ = True
    hub_lic = None
    inr_lic = None
    ...
    def test_sr_dyn_lic_groups(self):
        ...
        inr_device, ip_address_inr = \
            self.find_device(self.device1, self.device2, controller2)

        device_request_inr = UhpRequestsDriver(ip_address_inr)

        uhp_opt_inr = device_request_inr.get_uhp_options()
        nms_opt = nms_api.get_sr_license_options('sr_license:0')
        uhp_opt_inr1 = uhp_opt_inr.replace("S2", "")

        hub_device, ip_address_hub = \
            self.find_device(self.device1, self.device2, controller1)
        ...
        if uhp_opt_hub1.startswith('(NMS)Options:') and uhp_opt_inr1.startswith('(NMS)Options:'):
            test_api.fail(self, 'Dynamic license was assigned to both devices')

        if uhp_opt_hub1.split('(NMS)Options:')[-1].rstrip().lstrip() == nms_opt and \
                uhp_opt_inr1.split('(NMS)Options:')[-1].rstrip().lstrip() == nms_opt:
            test_api.fail(self, 'Dynamic license was assigned to both devices')
        ...
        if not nms_api.wait_up(self.mf_hub) or not nms_api.wait_up(self.inroute):
            test_api.fail(self, 'Controllers are not in Up state')

    def find_device(self, sr_device1, sr_device2, controller):
        # find active device
