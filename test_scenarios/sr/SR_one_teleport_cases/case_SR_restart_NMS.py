...
class SrRestartNmsCase(CustomTestCase):
    """Check that after NMS restart, network (MF-Hub and 2 stations) under SR ctrl is in UP state"""
    ...
    def test_sr_restart_nms(self):
        ...
        if state1 != state11 or state2 != state22:
            test_api.fail(self, 'Devices states have changed after restart NMS')
