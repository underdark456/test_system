...
class SrSetUpOutInrCase(CustomTestCase):
    """Set up Star topology (Outroute + Inroute) in Smart Redundancy. 3 Devices, 4 Stations"""
    ...
    def set_up_class(cls):
        ...
        cls.st0.star_station(params={
            'rx1_frq': cls.options.get('Outroute').get('tx_frq'),
            'rx1_sr': cls.options.get('Outroute').get('tx_sr'),
    ...
    def test_SR_set_up_MF(self):
        ...
        outroute = Controller.create(self.driver, net.get_id(), self.options.get('Outroute'))
        outroute.send_params({
            'binding': '2',
        ...
        inroute = Controller.create(self.driver, net.get_id(), self.options.get('Inroute'))
        inroute.send_params({
            'binding': '2',
            'sr_controller': f'sr_controller: {sr_ctrl.get_id()}',
            'dyn_license': '1',
        ...
        if not inroute.wait_up(timeout=240):
            raise NmsControlledModeException('Timeout! Controller Inroute is not in UP state')
        ...
        stn0 = Station.create(self.driver, vno_test.get_id(), {
            ...
            'rx_controller': f'controller:{inroute.get_id()}'
        })
        ...
        ControllerRoute.create(self.driver, outroute.get_id(), {
            'type': RouteTypes.IP_ADDRESS,
            'service': f'service:{service.get_id()}',
            'ip': '192.168.1.1'
        })
        ControllerRoute.create(self.driver, inroute.get_id(), {
            'type': RouteTypes.IP_ADDRESS,
            'service': f'service:{service.get_id()}',
            'ip': '192.168.1.2'
        })
        ...
        if states.count('Up') != 2 and states.count('Redundant') != 1:
            self.fail('Fault state! Two of the devices should be UP, while the third one is Redundant')

        # to check the availability of stations
        if sr_device1.get_param('state') == 'Up' and sr_device1.get_param('controller') == 'controller:0 test_ctrl_Out':
            ip_address = sr_device1.get_param('ip')
        elif sr_device2.get_param('state') == 'Up' \
                and sr_device2.get_param('controller') == 'controller:0 test_ctrl_Out':
            ip_address = sr_device2.get_param('ip')
        elif sr_device3.get_param('state') == 'Up' \
                and sr_device3.get_param('controller') == 'controller:0 test_ctrl_Out':
