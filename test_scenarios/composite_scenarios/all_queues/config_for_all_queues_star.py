...
class ConfigForAllQueuesStarCase(_AbstractCase):
    """Config creation for all queues traffic case in a Star network"""
    # Simple case that creates a config that features a star network with a hub and a station.
    ...
    def set_up_class(cls):
        ...
        cls.controller_ip = cls.options.get('controller_star').get('device_ip')
    ...
    def test_create_network(self):
        ...
        star_controller = Controller.create(self.driver, net.get_id(), params=self.options.get('controller_star'))
        vno = Vno.create(self.driver, net.get_id(), params=self.options.get('vno'))
        stn = Station.create(self.driver, vno.get_id(), params=self.options.get('station_star'))
        ...
        for i in range(7):
            ...
            net_tx_hub = ControllerRoute.create(self.driver, star_controller.get_id(), params={
            ...
            hub_vlan += 1
            stn_vlan += 1
        self.backup.create_backup('all_queues_policies_star.txt')
