...
from test_scenarios.balancer.load_type_tdma_total.base_config_2ctrl_1bal_total import _Base2Ctrl1BalTotalCase

options_path = 'test_scenarios.balancer'
backup_name = 'default_config.txt'


class BalancerBtw2TotalLowerCase(_Base2Ctrl1BalTotalCase):
    """The test to check 2 stations are switched due to load_lower limits 2.4.4"""
    ...
    def test_balancer_btw2_lower(self):

        nms_api.update('bal_controller:0', {'load_lower': '10',
                                            'load_higher': '60'})

        nms_api.update('station:3', {
            'rx_controller': self.mf_hub,
            'no_balancing': '1'
        })
        nms_api.update('station:4', {
            'rx_controller': self.mf_hub,
            'no_balancing': '1'
        })
        time.sleep(60)
        ...
        self.assertTrue(nms_api.wait_up('station:3'), msg='Station 4 is not in Up state after ctrl changing ')

        nms_api.update('station:4', {
            'no_balancing': '0'
        })
        self.assertTrue(nms_api.wait_up('station:4'), msg='Station 5 is not in Up state after ctrl changing ')
        ...
        st4_ctrl_act = nms_api.get_param('station:3', 'rx_ctr_act')
        st4_ctrl_set = nms_api.get_param('station:3', 'rx_controller')
        self.assertNotEqual(st4_ctrl_act, st4_ctrl_set)
        st5_ctrl_act = nms_api.get_param('station:4', 'rx_ctr_act')
        st5_ctrl_set = nms_api.get_param('station:4', 'rx_controller')
        self.assertNotEqual(st5_ctrl_act, st5_ctrl_set)
        # Check parameter Load_Balancing on Balancer ctrl dashboard (should be 2)
