...
from test_scenarios.balancer.load_type_tdma_total.base_config_3ctrl_1bal_total import _Base3Ctrl1TotalBalCase

options_path = 'test_scenarios.balancer'
backup_name = 'default_config.txt'


class BalancerReturnAllCase(_Base3Ctrl1TotalBalCase):
    """Check return_all after load balancing 2.5.3"""

    __author__ = 'vpetuhova'
    __version__ = '4.0.0.26'
    __review__ = ''
    __execution_time__ = 2000  # approximate test case execution time in seconds
    ctrl_telnet = None
    ...
    def test_balancer_return_all(self):
        ...
        while i < 10:
            for _ in range(3):
                ...
                self.fail(f'Station1 return_rate1 is unexpectedly lower than 300')
            time.sleep(60)
            nms_api.wait_ticks(10)

            # wait station Up
            self.assertTrue(nms_api.wait_up('station:0'), msg=f'Station1 state is not Up at iteration {i}')
            ...
            # Check parameter Load Balancing on Balancer ctrl dashboard (should be i)
            self.assertEqual(nms_api.get_param('bal_controller:0', 'load_sw'), i)

            nms_api.return_all('bal_controller:0')
            # wait station Up
            self.assertTrue(
                nms_api.wait_state('station:0', 'Down'),
                msg=f'Station1 state is not Down after return_all at iteration {i}'
            )
            ...
            # wait station Up
            self.assertTrue(
                nms_api.wait_up('station:0'),
                msg=f'Station1 state is not Up after return_all at iteration {i}'
            )
