...
from src.exceptions import NmsControlledModeException, ObjectNotUpdatedException

options_path = 'test_scenarios.form_confirmation.modcod'
backup_name = 'default_config.txt'


class AcmTdmUhp200xConfirmationCase(CustomTestCase):
    """UHP200X gets correct ACM TDM modcods"""

    __author__ = 'dkudryashov'
    __version__ = '4.0.0.20'
    __execution_time__ = 860  # approximate test case execution time in seconds
    __express__ = True

    @classmethod
    def set_up_class(cls):
        uhp200x_modems = test_api.get_uhp_by_model(ModelTypesStr.UHP200X, number=1)
        ...
        cls.class_logger.info(f'UHP {cls.uhp_ip} SW version: {uhp200x_modems[0].get("sw")}')
        ...
        tp = nms_api.create(net, 'teleport', params={
            'name': 'tp-0',
            'sat_name': 'sat-0',
            'tx_lo': 0,
            'rx1_lo': 0,
    ...
    def test_mf_hub_tdm_acm_sf(self):
        ...
        for i in range(len(self.sf_modcods) - 8):
            acm_thr = round(random.uniform(0, 5.9), 1)  # getting random float value for acm_thr parameter
            ...
            acm_mc7, acm_mc7_name = self.sf_modcods[i + 6].get('value'), self.sf_modcods[i + 6].get('name')
            acm_mc8, acm_mc8_name = self.sf_modcods[i + 7].get('value'), self.sf_modcods[i + 7].get('name')
            ...
            acm_mc7_cn = self.sf_modcods[i + 6].get('acm_thr')
            acm_mc8_cn = self.sf_modcods[i + 7].get('acm_thr')
            with self.subTest(f'tx_modcod={tx_modcod_name} ({tx_modcod_cn}dB), acm_mc2={acm_mc2_name} ({acm_mc2_cn}dB),'
                ...
                try:
                    ...
                    self.fail(f'MF hub is not updated, reason: {exc}')
    ...
    def test_mf_hub_tdm_acm_lf(self):
        """Check long frames combinations"""
        for i in range(len(self.lf_modcods) - 8):
            acm_thr = round(random.uniform(0, 5.9), 1)  # getting random valid acm_thr
            tx_modcod, tx_modcod_name = self.lf_modcods[i].get('value'), self.lf_modcods[i].get('name')
            tx_modcod_cn = self.lf_modcods[i].get('acm_thr')
            acm_mc2, acm_mc2_name = self.lf_modcods[i + 1].get('value'), self.lf_modcods[i + 1].get('name')
            acm_mc3, acm_mc3_name = self.lf_modcods[i + 2].get('value'), self.lf_modcods[i + 2].get('name')
            acm_mc4, acm_mc4_name = self.lf_modcods[i + 3].get('value'), self.lf_modcods[i + 3].get('name')
            acm_mc5, acm_mc5_name = self.lf_modcods[i + 4].get('value'), self.lf_modcods[i + 4].get('name')
            acm_mc6, acm_mc6_name = self.lf_modcods[i + 5].get('value'), self.lf_modcods[i + 5].get('name')
            acm_mc7, acm_mc7_name = self.lf_modcods[i + 6].get('value'), self.lf_modcods[i + 6].get('name')
            acm_mc8, acm_mc8_name = self.lf_modcods[i + 7].get('value'), self.lf_modcods[i + 7].get('name')
            acm_mc2_cn = self.lf_modcods[i + 1].get('acm_thr')
            acm_mc3_cn = self.lf_modcods[i + 2].get('acm_thr')
            acm_mc4_cn = self.lf_modcods[i + 3].get('acm_thr')
            acm_mc5_cn = self.lf_modcods[i + 4].get('acm_thr')
            acm_mc6_cn = self.lf_modcods[i + 5].get('acm_thr')
            acm_mc7_cn = self.lf_modcods[i + 6].get('acm_thr')
            acm_mc8_cn = self.lf_modcods[i + 7].get('acm_thr')
            with self.subTest(f'tx_modcod={tx_modcod_name} ({tx_modcod_cn}dB), acm_mc2={acm_mc2_name} ({acm_mc2_cn}dB),'
                              f' acm_mc3={acm_mc3_name} ({acm_mc3_cn}dB), acm_mc4={acm_mc4_name}, ({acm_mc4_cn}dB),'
                              f' acm_mc5={acm_mc5_name} ({acm_mc5_cn}dB), acm_mc6={acm_mc6_name}, ({acm_mc6_cn}dB),'
                              f' acm_mc7={acm_mc7_name} ({acm_mc7_cn}dB), acm_mc8={acm_mc8_name}, ({acm_mc8_cn}dB)'):
                ...
                for j in range(2, 9):
                    self.assertEqual(uhp_values.get(f'acm_mc{j}'), self.lf_modcods[i + j - 1].get("value"))
