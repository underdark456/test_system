...
class ConfigMFNetHub2Stn(CustomTestCase):
    """Configuring MF network: hub and two stations in NMS. Configuring UHP for MF network.
        Start MF network. Checking that all UHP are in operation"""
    system_options = None
    options = None
    stations = None
    __author__ = 'ish'
    ...
    def set_up_class(cls):
        ...
        cls.controller_nms = Controller.create(cls.driver, 0, {
            'name': 'mf_hub',
        ...
        # Configuring UHP
        cls.controller_uhp = controllers[0].get('web_driver')
        password = Network(cls.driver, 0, 0).read_param('dev_password')
        cls.controller_uhp.set_nms_permission(vlan=controllers[0].get('device_vlan'), password=password)

        cls.station_01_uhp = cls.stations[0].get('web_driver')
        cls.station_01_uhp.star_station(profile_number=2, params={
            'rx1_frq': cls.controller_nms.read_param('tx_frq'),
            'rx1_sr': cls.controller_nms.read_param('tx_sr'),
            'timeout': 120,
        })
        cls.station_02_uhp = cls.stations[1].get('web_driver')
        cls.station_02_uhp.star_station(profile_number=2, params={
            'rx1_frq': cls.controller_nms.read_param('tx_frq'),
            'rx1_sr': cls.controller_nms.read_param('tx_sr'),
            'timeout': 120,
        })
    ...
    def set_up(self):
        ...
        self.assertTrue(self.controller_nms.wait_up(), 'MF hub is not ready')
